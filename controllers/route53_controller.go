...
func (r *Route53Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	contextLogger := log.WithFields(log.Fields{
		"cr-name":   route53.Name,
		"cr-values": route53.Spec,
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			contextLogger.Info("route53 resource not found. Ignoring since object must be deleted")
			return ctrl.Result{}, nil
		}
		// Error reading the object - requeue the request.
		contextLogger.Error(err, "Failed to get memcached")
	...
	if !controllerutil.ContainsFinalizer(route53, route53Finalizer) {
		contextLogger.Info("Adding Finalizer for Route53")
		if ok := controllerutil.AddFinalizer(route53, route53Finalizer); !ok {
			contextLogger.Error(err, "Failed to add finalizer into the custom resource")
			return ctrl.Result{Requeue: true}, nil
		}

		if err = r.Update(ctx, route53); err != nil {
			contextLogger.Error(err, "Failed to update custom resource to add finalizer")
	...
	if isRoute53MarkedToBeDeleted {
		contextLogger.Info("marked to be deleted")
		if controllerutil.ContainsFinalizer(route53, route53Finalizer) {
			contextLogger.Info("Performing Finalizer Operations for Route53 before delete CR")

			// Let's add here an status "Downgrade" to define that this resource begin its process to be terminated.
			meta.SetStatusCondition(&route53.Status.Conditions, metav1.Condition{Type: typeDegradedRoute53,
				Status: metav1.ConditionUnknown, Reason: "Finalizing",
				Message: fmt.Sprint("Performing finalizer operations")})

			if err := r.Status().Update(ctx, route53); err != nil {
				contextLogger.Error(err, "Failed to update Route53 status")
				return ctrl.Result{}, err
			}
			...
			if err := r.Get(ctx, req.NamespacedName, route53); err != nil {
				contextLogger.Error(err, "Failed to re-fetch route53")
				return ctrl.Result{}, err
			}

			meta.SetStatusCondition(&route53.Status.Conditions, metav1.Condition{Type: typeDegradedRoute53,
				Status: metav1.ConditionTrue, Reason: "Finalizing",
				Message: fmt.Sprint("Finalizer operations were successfully accomplished")})
			...
			contextLogger.Info("Removing Finalizer for Route53 after successfully perform the operations")
			if ok := controllerutil.RemoveFinalizer(route53, route53Finalizer); !ok {
				contextLogger.Error(err, "Failed to remove finalizer for Route53")
				return ctrl.Result{Requeue: true}, nil
			}
